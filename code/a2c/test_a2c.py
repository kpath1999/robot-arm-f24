...
write_to = os.path.join(root, 'results', 'a2c')
...
# Set up logging
log_file = write_to + f'/a2c_log_{time.strftime("%Y%m%d-%H%M%S")}.csv'
...
if not os.path.exists(log_file):
    with open(log_file, mode='w', newline='') as file:
        ...
        writer.writerow(["Episode", "Epoch", "Mean Episode Length", "Entropy", "Value Error", "Gradient Norm", "Clip Loss", "Value Loss", "Action Mean", "Action Std"])

def log_metrics(episode, episode_length, entropy, value_error, gradient_norm, 
                clip_loss, value_loss, action_mean, action_std):
    # Write metrics to CSV log
    with open(log_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([episode, epoch, episode_length, entropy, value_error, gradient_norm, clip_loss, value_loss, action_mean, action_std])
...
PATH = write_to + f'/a2c_model_{time.strftime("%Y%m%d-%H%M%S")}.pt'
...
for s in range(season):
    policy.eval()
    old_probs_lst, states_lst, actions_lst, rewards_lst, values_lst, dones_list = collect_trajectories(envs=envs, policy=policy, tmax=tmax, nrand = 5)
    ...
    for epoch in range(opt_epoch):
        for b in range(n_sample):
            ...
            # Policy statistics
            mean_action_probs = action_est.mean().item()
            std_action_probs = action_est.std().item()

            # Value function statistics
            mean_values = values.mean().item()
            std_values = values.std().item()
            value_mse = ((tv - values).pow(2)).mean().item()

            # Performance metrics
            mean_episode_length = tmax - dones_list.float().sum(dim=0).mean().item()
            ...
            # Log the metrics (to a csv)
            log_metrics(s, epoch, mean_episode_length, S.item(), value_mse, total_norm, L_CLIP.item(), L_VF.item(), mean_action_probs, std_action_probs)
