## https://github.com/mahyaret/kuka_rl/tree/master
...
# TODO: Add logging capabilities so that others can see training progress
...
def get_screen():
    ...
    # Convert to float, rescale, convert to torch tensor (doesn't require a copy)
...
# Get screen size so that we can initialize layers correctly based on shape (returned screen is 48x48x3)
...
def optimize_model():
    ...
    # Converting batch-array of transitions to transition of batch arrays
...
mod_dir = os.path.join(current_dir, 'models')

os.makedirs(mod_dir, exist_ok=True)

PATH = os.path.join(mod_dir, 'kuka_policy_dqn.pth')
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        stacked_states_t = torch.cat(list(stacked_states), dim=1)
        ...
        if done:
            ...
            if (best_mean_reward is None or best_mean_reward < mean_reward) and i_episode > 100:
                ...
                torch.save({'policy_net_state_dict': policy_net.state_dict(),
                            ...
                            'optimizer_policy_net_state_dict': optimizer.state_dict()}, PATH)
                
                if best_mean_reward is not None:
                    print('Best mean reward updated %.1f -> %.1f' % (best_mean_reward, mean_reward))
    ...
    if i_episode >= 200 and mean_reward > 50:
        writer.add_scalar('Environment solved in episodes', i_episode, mean_reward)
        print('Environment solved in {:d} episodes!\tAverage Score: {:.2f}'.format(i_episode+1, mean_reward))
        break

writer.add_scalar('Average Score', mean_reward, i_episode)
print('Average Score: {:.2f}'.format(mean_reward))
elapsed = timeit.default_timer() - start_time
writer.add_scalar('Elapsed time', elapsed, i_episode)
